...
def _sec_feed_html(feed: pd.DataFrame) -> str:
    records = []
        ...
        records.append(
            {
                ...
                "headline_short": extract_feed_headline(row.headline),
                "filing_url": getattr(row, "filing_url", ""),
                "form_type": getattr(row, "form_type", ""),
                "sentiment": getattr(row, "sentiment", "") or infer_sentiment(row.headline),
            }
        )
    payload = _json_script_payload(records)
    return f"""
...
<style>
  ...
  tr.feed-very-bullish {{ color: #064e3b; }}
  tr.feed-bullish {{ color: #16a34a; }}
  tr.feed-neutral {{ color: #111827; }}
  tr.feed-little-bearish {{ color: #a16207; }}
  tr.feed-very-bearish {{ color: #7f1d1d; }}
  .headline-container {{
    display: flex;
    align-items: center;
    gap: 12px;
  ...
  .very-bullish-dot {{ background: #064e3b; }}
  .bullish-dot {{ background: #22c55e; }}
  .neutral-dot {{ background: #6b7280; }}
  .little-bearish-dot {{ background: #facc15; }}
  .very-bearish-dot {{ background: #7f1d1d; }}
  .pagination {{
    display: flex;
...
<body>
<div class="top-bar"><h1>SEC Stock Feed</h1></div>
<div class="search-box">
  <input type="text" id="search-input" placeholder="Search ticker, company, or headline..." />
  <select id="form-filter" aria-label="Filter by SEC form type">
...
<script id="feed-data" type="application/json">{payload}</script>
<script>
const allStocks = JSON.parse(document.getElementById('feed-data').textContent || '[]');
...
function sentimentClass(sentiment) {{
  ...
  if (text.includes('very very bullish') || text.includes('extremely bullish') || text.includes('very bullish') || text.includes('strongly bullish')) return 'very-bullish-dot';
  if (text.includes('very very bearish') || text.includes('extremely bearish') || text.includes('very bearish') || text.includes('strongly bearish')) return 'very-bearish-dot';
  if (text.includes('little bearish') || text.includes('slightly bearish') || text.includes('mildly bearish')) return 'little-bearish-dot';
  if (text.includes('bullish')) return 'bullish-dot';
  if (text.includes('bearish') || text.includes('negative')) return 'very-bearish-dot';
  return 'neutral-dot';
}}

function rowSentimentClass(sentiment) {{
  const text = String(sentiment || '').toLowerCase();
  if (text.includes('very very bullish') || text.includes('extremely bullish') || text.includes('very bullish') || text.includes('strongly bullish')) return 'feed-very-bullish';
  if (text.includes('very very bearish') || text.includes('extremely bearish') || text.includes('very bearish') || text.includes('strongly bearish')) return 'feed-very-bearish';
  if (text.includes('little bearish') || text.includes('slightly bearish') || text.includes('mildly bearish')) return 'feed-little-bearish';
  if (text.includes('bullish')) return 'feed-bullish';
  if (text.includes('bearish') || text.includes('negative')) return 'feed-very-bearish';
  return 'feed-neutral';
}}
...
function parseAnalysis(text) {{
  ...
  function pushCurrent() {{
    if (currentKey) rows.push({{ key: currentKey, value: currentValue.join('\\n').trim() }});
  }}
  for (const line of lines) {{
    const itemMatch = line.match(/^Item\\s+([^\\s]+)\\s*(.*)$/i);
    if (itemMatch) {{
      pushCurrent();
      currentKey = `Item ${{itemMatch[1]}}`;
      currentValue = itemMatch[2] ? [itemMatch[2].replace(/^[:\\-]\\s*/, '')] : [];
      continue;
    }}
    const fieldMatch = line.match(/^([A-Za-z][A-Za-z /&()%-]{1,55})\\s*:\\s*(.*)$/);
    if (fieldMatch) {{
      pushCurrent();
  ...
  pushCurrent();
  if (!rows.length && raw.trim()) rows.push({{ key: 'Analysis', value: cleanMarkdown(raw) }});
  return rows.filter(row => row.key && row.value);
}}

function analysisHeadline(stock) {{
  const rows = parseAnalysis(stock.headline);
  const headlineRow = rows.find(row => row.key.toLowerCase() === 'headline' || row.key.toLowerCase() === 'short headline');
  if (headlineRow && headlineRow.value) return headlineRow.value;
  return stock.headline_short || 'No headline';
}}

function analysisTableHtml(text) {{
  const rows = parseAnalysis(text);
  if (!rows.length) return '<span>No analysis available.</span>';
  return `<table class="analysis-table"><tbody>${{rows.map(row => `
    <tr>
...
function renderTable() {{
  ...
  const currentItems = filteredStocks.slice(start, start + ITEMS_PER_PAGE);
  currentItems.forEach(stock => {{
    const row = document.createElement('tr');
    row.className = rowSentimentClass(stock.sentiment);
    const headline = escapeHtml(analysisHeadline(stock));
    const filingUrl = String(stock.filing_url || '');
    const headlineHtml = `<a href="#" class="headline-link">${{headline}}</a>`;
    const filingHtml = filingUrl
      ? `<div style="margin-top: 8px;"><a href="${{escapeHtml(filingUrl)}}" target="_blank" rel="noopener" class="headline-link" style="font-size: 12px; font-weight: 600;">Open SEC filing</a></div>`
      : '';
    row.innerHTML = `
      <td>${{escapeHtml(stock.datetime || '')}}</td>
      <td><strong>${{escapeHtml(stock.form_type || '')}}</strong></td>
      <td><strong>${{escapeHtml(stock.ticker || '')}}</strong></td>
      <td>${{escapeHtml(stock.company_name || '')}}</td>
      <td>
        <div class="headline-container">
          <span class="sentiment-dot ${{sentimentClass(stock.sentiment)}}"></span>
          ${{headlineHtml}}
        </div>
        ${{filingHtml}}
        <div class="analysis-content">${{analysisTableHtml(stock.headline || '')}}</div>
      </td>
    `;
    tableBody.appendChild(row);
  }});
...
document.addEventListener('click', event => {{
  const link = event.target.closest('.headline-link');
  if (!link) return;
  if (link.getAttribute('target') === '_blank') return;
  event.preventDefault();
  const content = link.closest('td').querySelector('.analysis-content');
...
function applyFilters() {{
  ...
  filteredStocks = allStocks.filter(stock => {{
    const matchesSearch = [stock.ticker, stock.company_name, stock.headline, stock.form_type].some(value => String(value || '').toLowerCase().includes(term));
    const matchesForm = selectedForm === 'all' || String(stock.form_type || '').toUpperCase() === selectedForm.toUpperCase();
...
def _sec_feed_autorefresh_html(seconds: int) -> str:
    milliseconds = max(10, int(seconds)) * 1000
    return f"""
    <script>
      window.setTimeout(() => {{
        const url = new URL(window.parent.location.href);
        url.searchParams.set('page', 'Stock Tracker');
        url.searchParams.set('stock_tracker_section', 'SEC Stock Feed');
...
def stock_tracker_page() -> None:
    ...
    stock_sections = ["Stock Tracker", "Earnings Calendar", "SEC Stock Feed"]
    ...
    elif selected_section == "Earnings Calendar":
        _render_earnings_calendar_tab()
    else:
        with st.expander("SEC Feed Controls", expanded=False):
            st.caption("The feed view below keeps the imported HTML design and includes its own search box.")
            limit = st.slider("Rows to load", min_value=25, max_value=500, value=250, step=25)
            auto_refresh = st.checkbox("Auto-refresh SEC feed page", value=True, key="stock_tracker_sec_feed_auto_refresh")
            refresh_seconds = st.number_input(
                ...
                value=60,
                step=10,
                key="stock_tracker_sec_feed_refresh_seconds",
                disabled=not auto_refresh,
            )

        feed = list_sec_feed(limit=limit)
        if feed.empty:
            st.write("No feed rows found.")
        else:
            if auto_refresh:
                components.html(_sec_feed_autorefresh_html(int(refresh_seconds)), height=0)
            components.html(_sec_feed_html(feed), height=900, scrolling=True)


def sec_stock_feeds_page() -> None:
    if not tracking_database_exists():
        st.warning("Stock tracking database was not found in data/stock_tracking/stocks.db.")
        return

    with st.expander("SEC Feed Controls", expanded=False):
        st.caption("The feed view below keeps the original HTML design and includes its own search box.")
        limit = st.slider("Rows to load", min_value=25, max_value=500, value=250, step=25)
        auto_refresh = st.checkbox("Auto-refresh SEC feed page", value=True, key="standalone_sec_feed_auto_refresh")
        refresh_seconds = st.number_input(
            "Refresh every seconds",
            min_value=10,
            max_value=600,
            value=60,
            step=10,
            key="standalone_sec_feed_refresh_seconds",
            disabled=not auto_refresh,
        )
